...
foreach (var type in types) {
    ...
    m(@"    field = value;");
    m(@"    NotifyPropertyChanged(propertyName);");
    m(@"    if (dependentPropertyNames != null) {");
    m(@"        foreach (var dependentPropertyName in dependentPropertyNames) {");
    m(@"            NotifyPropertyChanged(dependentPropertyName);");
    m(@"        }");
    m(@"    }");
    m(@"    return true;");
    m(@"}");
    ...
    m(@"    field = value;");
    m(@"    OnPropertyChanged(e);");
    m(@"    if (dependents != null) {");
    m(@"        foreach (var dependent in dependents) {");
    m(@"            OnPropertyChanged(dependent);");
    m(@"        }");
    m(@"    }");
    m(@"    return true;");
    m(@"}");
